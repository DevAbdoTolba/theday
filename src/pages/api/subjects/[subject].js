...
const getCachedSubjectData = unstable_cache(
  async (subject) => {
    // Your existing GetDataOfSubject function here
    ...
    return await GetDataOfSubject(subject);
  },
  ["subject-data"],
...
export default async function handler(req, res) {
  ...
  try {
    let GetDataOfSubjectData;

    if (cache === "1") {
      // Force update the cache
      GetDataOfSubjectData = await getCachedSubjectData.mutate(subject);
    } else {
      // Use the cached value or fetch if not available
      GetDataOfSubjectData = await getCachedSubjectData(subject);
    }
